...
void mybt_progress(void)
{
	...
	if(simulate_temp) {
		...
		bt_gatt_notify(NULL, &service1.attrs[MYGATT_SRV2_CH0_CHRC0], &values.voltage[0], sizeof(values.voltage[0]));
		bt_gatt_notify(NULL, &service1.attrs[MYGATT_SRV2_CH1_CHRC0], &values.voltage[1], sizeof(values.voltage[1]));
